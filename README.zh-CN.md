...
harness 将项目知识划分为四类，每一类都有明确的归属。**不要混在一起维护。**
...
整理成一个 skill 文件。`test-architect` 在设计新测试之前，会扫描目标项目的 `.claude/skills/` 目录，
当关键词匹配时，优先复用这些文件中的代码模板，而不是从头重新发明流程。
...
docs/coding-rules.md         coding-rules ��考模板（供目标项目复制和改造）
