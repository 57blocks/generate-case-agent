...
- analyst 的**澄清问题**（仅在必要时出现）
- architect 的**破坏性变更警告**（当新方案与现有代码冲突时出现）
- runner 的**迭代更新信息**（例如：`Iteration 2: fix selector issue…`）
- 最后一份**会话总结**，说明实现了什么、复用了什么，以及向 `CLAUDE.md` 回写了哪些经验

### 自动迭代

当测试失败时，`test-runner` 会读取 stack trace，对失败进行分类，然后：

- 直接修复 selector、timing 或断言问题，或
...
harness 将项目知识划分为 4 类，每一类都有明确的归属。**不要混在一起维护。**
...
| **C. 业务流程代码模板** | “如何登录 + 创建记录 + 验证”；“如何上传文件 + 等待 AI 处理 + 发布” | **目标项目自己的** `.claude/skills/*.md`（本 harness 仅提供一个示例） |
| **D. 真实可运行的代码示例** | 一条通过的 spec 及其 Page Object | 你项目中现有的 `tests/` 和 `pages/` —— architect 会自行 grep 并参考 |

> 这个 harness 仓库只提供 **A**（模板）和 **C**（一个示例）。**B** 完全属于你的项目，
> 而 **D** 应直接从你的代码库中读取，无需额外复制出来。

### Skill 的定位（C 类）

`example-flow.md` 是一份**模板**，用于说明一个 skill 应该长什么样。当某类业务流程会被多条测试
重复使用（例如“登录 → 创建 → 校验”），你应当把这段流程中的**真实代码片段、命名约定、参数解析**
整理成一个 skill 文件。`test-architect` 在设计新测试之前，会扫描目标项目的 `.claude/skills/`，
当关键词匹配时，优先复用 skill 中的代码模板，而不是从头重新发明流程。
...
`{ROLE}`、`{fixture1}` 的占位符。**这些并不是让你手动替换的**，而是给 LLM agent 使用的模板。
...
| 项目编码风格（断言、loading 等待） | 读取 `docs/coding-rules.md` + `CLAUDE.md` | 至少提供其中一份，并写清楚规范 | architect 会退回到通用规则 |
...
- 角色枚举（`utils/constants.ts` 或类似文件）—— 没有的话，每次都要手动告诉 analyst role 是什么
- Page Object 目录 —— 没有的话，新方法可能会被放到奇怪的位置
- `CLAUDE.md` —— 没有的话，architect 缺乏项目上下文，准确率会明显下降
- `docs/coding-rules.md` —— 没有的话，architect 会退回到通用 Playwright 规则，导致组件库特定的等待 / selector 写法不准确
...
- 一份 `docs/coding-rules.md`（可以从 harness 提供的模板开始 —— 见 [`docs/coding-rules.md`](docs/coding-rules.md)）
...
| Cursor | ⚠️ 可改造 | 需要把每个 `.md` agent 改写成 `.cursor/rules/*.mdc`，手动触发各阶段，并接受失去自动编排 |
| GitHub Copilot | ❌ 不实用 | 没有 sub-agent，也没有 MCP，准确率会大幅下降 |

### 改造到 Cursor

Cursor 的 Composer Agent 是 Claude Code 在其他 IDE 里最接近的对应物，但它**不支持 sub-agent**。
一个大致的迁移路径如下：

1. 将 `.claude/agents/*.md` 重写为 `.cursor/rules/*.mdc`。去掉 frontmatter，保留主要 prompt。
2. 在 `~/.cursor/mcp.json` 中配置 MCP Playwright server，使 architect 仍然可以验证 selector。
3. 在 Composer 中手动触发各阶段：`@analyst` → `@architect` → `@coder` → `@runner`。
   这样会失去**自动失败迭代**，需要你手动重新触发 `@runner`（设计问题则回到 `@architect`）。
4. 保留 `/tmp/tc_*.md` 这套 artifact 协议 —— 任何可以访问文件系统的 IDE 都可以使用。

因此你会失去：自动阶段路由、自动失败迭代，以及按阶段切换模型的能力（Claude Code 中的
`model: haiku` / `model: sonnet`）。

### 改造到 Copilot

不推荐。Copilot Chat 没有 sub-agent 概念，Copilot Workspace 也不支持 MCP。
这套流水线的准确率高度依赖 **MCP 实时 selector 验证**。没有它，architect 只能根据 class 名
“猜” selector，而 runner 容易陷入那种“没有真正修复方案”的 selector 错误循环。

如果一定要尝试，可以把 5 个阶段的 prompt 合并为一个超长 prompt 交给 Copilot Agent，
但需要接受准确率明显下降。
...
- **代码风格** —— agent 会读取目标项目的 `docs/coding-rules.md`。你可以在那里（或你项目的 `CLAUDE.md` 中）写规则，用来教会 harness 你的约定。
...
- **仅支持 Linux / macOS** —— artifact 使用 `/tmp/`。Windows 用户需要自行修改各 agent 中的路径。
